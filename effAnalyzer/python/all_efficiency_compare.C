...
void all_efficiency_compare(){
    ...
    leg->AddEntry(h_pt_eff,"GGToMuMu-MC","lep");
    leg->AddEntry(h_pt_eff_data,"InclusiveJpsi-MC","lep");
    leg->Draw();
    c_pt->Update();
 ...
 TH1F* h_eta_denom_mc = (TH1F*) fIn1->Get("eff/h_eta_probe_all");
   ...
   h_eta_eff_data->SetMarkerColor(kBlue);
    ...
    legeta->AddEntry(h_eta_eff_mc,"GGToMuMu-MC","lep");
    legeta->AddEntry(h_eta_eff_data,"InclusiveJpsi-MC","lep");
    legeta->Draw();
    c_eta->Update();
...
TH1F* h_phi_denom_mc = (TH1F*) fIn1->Get("eff/h_phi_probe_all");
   ...
   legphi->AddEntry(h_phi_eff_mc,"GGToMuMu-MC","lep");
   legphi->AddEntry(h_phi_eff_data,"InclusiveJpsi-MC","lep");
   legphi->Draw();
   c_phi->Update();
   
   TFile *outf = TFile::Open("eff_plot_compare.root","RECREATE");
   c_pt->Write();
   c_eta->Write();
   c_phi->Write();
   outf->Close();
